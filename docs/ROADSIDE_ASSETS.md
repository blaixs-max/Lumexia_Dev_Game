...
## Güncel ek çalışma — 7 Eylül 2026

Yaklaşırken değişen cephe ve bitki görüntüsü için pencere/perde yüzeyleri ayrıldı, cephe kalınlıkları hesaba katıldı ve yaprak alfa eşiği 0,30/MSAA kapsamına taşındı. Çam iğne geometrisi ağaç başına 17.584 → **12.184 üçgene** indirildi (%30,71). Güncel toplam **18 prototip / 58.540 üçgendir**; aşağıdaki tablo yeni fabrika çıktısını gösterir. Önceki stüdyo kaydı 68.152 üçgendi ve tarihsel ölçüm olarak aşağıda korunur.

Çevre artık yalnız görünür örnekleri GPU'ya gönderir. Binalar ve ağaçlar için sis/görünürlük kalite seçeneklerinde aynıdır; Auto çalışma sırasında yalnız çözünürlüğü ayarlar. Kare süresi karşılaştırmasının yöntemi ve sonuç durumu [PERFORMANCE_REVIEW.md](PERFORMANCE_REVIEW.md) içindedir; model üçgen sayısı FPS ölçümü değildir.
...
`RaceScene`, yol kenarı için artık `RoadsideWorld` bileşenini kullanır. Bu çevre akışı önceki bina, ağaç ve çiftlik GLTF dosyalarını yüklemez. Önceki dosyaların depoda bulunması veya atıflarının korunması, yeni çevrenin onları kullandığı anlamına gelmez. Oyuncu ve trafik araçlarının güncel ölçümleri ayrı [model kaydındadır](../public/models/RUNTIME_MODELS.md).

| Grup / prototip | Adet | Bir prototipte üçgen | Malzeme çizimi |
|---|---:|---:|---:|
| `townhouse` — sıvalı, tuğla kaideli iki katlı ev | 1 | 1.786 | 11 |
| `villa` — teraslı modern konut | 1 | 1.630 | 10 |
| `apartment` — balkonlu altı katlı apartman | 1 | 10.942 | 11 |
| `office` — on katlı cam cepheli ofis | 1 | 6.156 | 8 |
| `warehouse` — tuğla atölye ve yükleme kapıları | 1 | 1.944 | 11 |
| `oak`, `oak2` — geniş taçlı meşe | 2 | 2.326 | 2 |
| `poplar`, `poplar2` — dar taçlı kavak | 2 | 2.374 | 2 |
| `pine`, `pine2` — iğne yaprak geometrili çam | 2 | 12.184 | 2 |
...
Perde ve cam aynı pencere alanını paylaşırken üst üste bindirilmez; kapı camı gerçek bir açıklığa yerleştirilir. Apartman pencerelerinin konumu duvar kaplamalarının kalınlığına göre belirlenir. Bu geometrik ayrım, yakın yüzeyler arasında derinlik belirsizliğini azaltır; kamera da 0,5–750 m aralığını kullanır.
...
Geometri tamponlarında eşleşen vertex özniteliklerinin indekslenmesi ayrıca uygulandı. Ölçülen 5 mimari + 9 doğa prototipinde vertex sayısı 172.362 → 115.455 (%33,02), öznitelik + indeks tamponu 6.775.440 → 4.790.700 bayt (%29,29) oldu; dört sokak detayı bu indeksleme ölçümünün dışındadır. Açılmış üçgen öznitelikleri karşılaştırmada korundu; lambada en büyük kayan nokta farkı 4,7 × 10⁻¹⁷'nin altındaydı. Bu geometri tamponu hesabı, toplam GPU belleği veya FPS ölçümü değildir.
...
| [`src/environment/visibility.js`](../src/environment/visibility.js) | Tam model sınır küresi, kamera hacmi ve 720 m dünya sarımı |
...
Yerleşimler seed tabanlıdır; render sırasında rastgele bir dünya yeniden oluşturulmaz. Aynı geometri/malzeme çok sayıda nesnede instancing ile paylaşılır. Görünür örnekler matris tamponunun başına yazılır ve `mesh.count` ile sınırlandırılır; görünmez örnekler sıfır ölçekle çizdirilmez. `DynamicDrawUsage` ve güncellenen tampon öneki kullanılır. Kamera ve dünya hareket etmediğinde matris hesabı/aktarımı atlanır.

Görünürlük denetimi bütün modelin ölçeklenmiş/döndürülmüş sınır küresini kullanır; pencere, gövde ve yaprak parçaları ayrı ayrı kaybolmaz. Bina ve ağaç yerleşimi bütün kalite seçeneklerinde korunur; sis 260–600 m, eleme sınırı 620 m'dir. Performance başlangıçta yalnız ot/çalı/çit gibi küçük detayları azaltır. Auto yarış ortasında yerleşimi değiştirmez. Çevrenin dinamik gölge çizimi kapalıdır; yumuşak temas izleri korunur. Bu düzenleme ölçülmüş telefon FPS artışı olarak sunulmaz.
...
- Son yerel lint ve üretim build kontrolleri geçti; 40 test geçti, mock modundaki 17 üretim testi atlandı. Güncel fabrika ölçümü 18 model/58.540 üçgendir; indeksli geometride üçgen sayısı indeks sayısından hesaplandı. Yedi görünürlük testi sınır kürelerini ve dünya sarımını doğrular. Bu kayıt yeni bir GitHub Actions/CI başarısı veya gerçek telefon testi değildir.
- Önceki çevre aşamasında stüdyo DOM kaydında 18 model, sonlu koordinatlar ve toplam 68.152 prototip üçgeni doğrulanmış; 390 × 844 mobil tarayıcı görünümü kontrol edilmişti. O aşamanın test sayısı 33 başarılı/17 atlanandı; güncel ölçümlerin yerine geçmez.
